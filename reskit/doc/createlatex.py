...
codeTypes = ["reskit.", "rydbergs", "hartrees", "eVs", "chart", "mcsmatfit",\
             "archive_root", "AsymCalc", "dMat", "cMatSympypolyk",\
             "cSMatSympypolyk","cSmat","dSmat","dKmat","dTmat",\
             "cFinMatSympypolyk", "mpmath", ".mpf", ".mpc"]
...
def code_font_str(string):
    ...
    for repStr in subcodeTypes:
        string = string.replace("}"+repStr,"}\codefont{"+repStr+"}")
    string = string.replace("use_\codefont{mpmath}_types",
                            "use_mpmath_types")
    return string
...
desc_str = (
r"The interface to \codefont{reskit} is through the \codefont{\_\_init\_\_.py} "
r"package file shown in figure~\ref{fig:ClassDiagram}. This section provides a "
r"summary of the available functions. Some of these are employed when a discrete "
r"set of scattering data (generated by other codes) is used and some when an "
r"functional expression is provided for these data. The parameters of each of "
